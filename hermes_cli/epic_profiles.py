...
Because that service does not exist yet, no Phase 0 profile is granted the
``terminal`` toolset: it resolves to ``terminal``+``process``, an unrestricted
shell that reaches ``git merge``, ``git push`` and any deploy script, which
would defeat the ``merge``/``deploy`` denials the same specs publish. Roles
needing command execution name it as a future service instead, exactly as
``verifier`` does for bounded test execution.
"""
...
SHADOW_PROFILE_SPECS = {
    ...
    "implementer": ShadowProfileSpec(
        "implementer",
        "One ticket writer in one assigned isolated worktree; bounded command "
        "execution is a future sandbox/write-reservation service. Phase 0's "
        "`kanban` toolset is coarse — it bundles worker handoffs with "
        "review-verdict and orchestrator fan-out tools — so this role is "
        "over-granted until a worker-scoped kanban subset exists. It therefore "
        "does not claim to deny the review verdict its own grant confers.",
        ("artifact_read", "file", "kanban"),
        ...
        ("unrelated worktree writes", "merge", "deploy"),
    ),
    "integration-writer": ShadowProfileSpec(
        "integration-writer",
        "Serialized writer for one isolated integration worktree; ticket "
        "lifecycle stays with the implementer and command execution is a "
        "future sandbox/write-reservation service.",
        ("artifact_read", "file"),
