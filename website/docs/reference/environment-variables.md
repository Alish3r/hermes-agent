...
| `HERMES_API_CALL_STALE_TIMEOUT` | Non-streaming stale-call timeout in seconds (default: `90`). Auto-disabled for local providers when left unset, and may scale upward for very large contexts. Known reasoning models get the same built-in floor as the streaming detector (see `HERMES_STREAM_STALE_TIMEOUT`). Also configurable via `providers.<id>.stale_timeout_seconds` or `providers.<id>.models.<model>.stale_timeout_seconds` in `config.yaml`. |
...
| `HERMES_STREAM_STALE_TIMEOUT` | Stale stream detection timeout in seconds (default: `180`). Auto-disabled for local providers. Triggers connection kill if no chunks arrive within this window. Known reasoning models get a higher built-in **floor** (180-600s, see `agent/reasoning_timeouts.py`) because their thinking phase emits no content token for minutes — e.g. Kimi K3 was measured at 228-333s. The floor only ever raises the timeout, and an explicit `providers.<id>.models.<model>.stale_timeout_seconds` always wins. |
