...
delegation:
  ...
  # route_allow_list:                       # Executor allow-list of provider:model routes for delegated children. null/absent (default) = every route allowed. Keep it null, not [] — an empty list refuses every route.
...
**Route allow-list (`route_allow_list`):** `delegation.route_allow_list` restricts which provider/model routes a delegated child may run on. Absent or `null` (the default) allows every route — the historical behavior; a list allows only the exact pairs it names, written as `"provider:model"` strings, two-item `[provider, model]` pairs, or `{provider: ..., model: ...}` mappings (matching is exact, with no case folding or trimming, and an entry that does not name both a provider and a model is ignored). The child's inherited fallback chain is pruned to the list before it runs, and a malformed non-list value fails closed: every route is refused and an error is logged. A refused child returns a failed result whose `route.status` is `refused`; the decision is recorded in the profile-local ledger `~/.hermes/governance/routes.db`, which stores route facts only — never prompts or task content.
